...
source /usr/local/bin/monitor_XLX_pt-BR_data
...
sudo journalctl -u xlxd.service -f | while read -r LINE; do
    ...
    if [[ "$LINE" =~ $REGEX_GATEKEEPER ]]; then
        ...
        ACAO_FORMATADA=$([[ "$ACAO" == "linking" ]] && echo "Tentativa de conexão no $REF_NAME" || echo "Tentativa de transmissão no $REF_NAME")
        if [ -z "$SUFIXO" ]; then
    ...
    # Verifica conexão de repetidoras
        ...
        TIMESTAMP_FORMATTED=$(format_timestamp "$TIMESTAMP_ORIGINAL")
        MESSAGE=$(format_message "$TIMESTAMP_FORMATTED" "$INDICATIVO" "$SUFIXO" "$IP" "$PROTOCOLO" "Conectou-se no $REF_NAME" "$MODULO")
        send_telegram_message "$MESSAGE" "$INDICATIVO"
    ...
    # Verifica desconexão de repetidoras
        ...
        TIMESTAMP_FORMATTED=$(format_timestamp "$TIMESTAMP_ORIGINAL")
        MESSAGE=$(format_message "$TIMESTAMP_FORMATTED" "$INDICATIVO" "$SUFIXO" "$IP" "$PROTOCOLO" "Desconectou-se do $REF_NAME" "$MODULO")
        send_telegram_message "$MESSAGE" "$INDICATIVO"
